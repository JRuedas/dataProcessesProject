...
Nowadays, pollution levels in Madrid is a popular topic in the news (click [here]((https://elpais.com/elpais/2019/11/29/inenglish/1575050927_871123.html)) for more information on the high pollution levels that Madrid's air is facing). This has worried some of the group members who are thinking about moving to Madrid because they live in the outskirts. That is why we want to know which area of Madrid is the most suitable for us to buy an apartment.
...
This report starts with an **exploratory data analysis**, where at first, before performing any analysis, our data is described. Then we explain which features are the most relevant ones for the issues that we concern about, and at last we explain the wrangling carried out to deal with our data. After this previous step, we go through several visuals that show how our data behaves in terms of center, dispersion and distribution measures. The exploratory analysis is ended with some previous insights about the temporal evolution of our data, before starting with further analysis.

This step is followed by a **predictive analysis first step**, in which we have used **temporal series** to predict the behavior of one of the features in our data in the year 2018, using the ETS predictive model.

After this first step, the second method used to ellaborate a prediction with our data uses a **machine learning approach**, in which we try to predict the outcome of one of our features based how it behaved previously, using the date and the station where the data was obtained as predictors.

To end this work, we present some **conclussions and future work** that could be carried out to follow this research.

We provide the **scripts** used to perform the various analysis and to generate the plots. They are described below:
...
In the next step of the research, after analysing the linear relationships of the variables and seeing that its very possible that none of them can work as a predictor to the others, a time series prediction model has been developed to check if we are able to accurately predict the behavior of O3 in the year 2018: the residuals plot shows that the model fits well and can predict the behavior correctly. For the issue that we concern about, which is moving to Madrid's city center, this time series itself is not useful, as we are predicting events that have already happened. However, this model could be used as a base to develop a more complex time series that could predict the behavior of the pollutants in upcoming years: that would be helpful to finally determine whether moving to Madrid's city center is a good idea or not.

--Machine learning conclussions paragraph

As an overall lesson, in future research on this domain using this data, it is convenient that additional variables that can work as predictors are used in the analysis. For example, the level of traffic in the city could be a good predictor to determine the growth of SO2 and CO, while the level of sunlight could be another predictor for O3. This would allow future contributors that are interested on this domain to perform a prediction based on factors other than time evolution. To be more precise in what would be exactly the best place to move in Madrid, it would also be a good idea to include some information about the districts in future analysis, which we didn't include to avoid increasing the complexity of our work, that was going to be mainly focused on temporal evolution since the beginning, due to our data.
